...
def parse_batch(line):
    ...
    t1..t3  stroke start in ms from the batch start
    ang     catch angle, degrees
    pow     peak acceleration x10

    Returns {rower: {"T":[...], "A":x, "P":y}} or None if malformed.
    """
...
def worst_from_batch(b):
    """Same comparison as find_worst, but on a batch the ESP32 already built.

    Arithmetic only. It does not decide whether the deviation matters.
    """
    avg = {}
    for r in (1, 2, 3):
        # offsets from whoever started each stroke first, in centiseconds
        offs = []
        for i in range(3):
            first = min(b[x]["T"][i] for x in (1, 2, 3))
