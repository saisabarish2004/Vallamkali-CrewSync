...
const float CATCH_ACC  = 0.8;
const float CATCH_GYRO = 25.0;
...
// One threshold, one minimum gap. No state machine, so nothing can get
// stuck. Motion above the threshold starts a stroke; anything within
// MIN_STROKE_PERIOD of the last one is the same stroke, not a new one.
// That single rule is what stops double packets.
void checkStroke() {
  unsigned long now = millis();

  // keep the peaks for the stroke in progress
  ...
  if (dynAcc < CATCH_ACC && gyroMag < CATCH_GYRO) return;   // not moving
  if (now - prevCatchTime < MIN_STROKE_PERIOD)    return;   // same stroke

  if (prevCatchTime != 0) {
    unsigned long period = now - prevCatchTime;
    if (period <= MAX_STROKE_PERIOD) {
      sendStrokePacket(period);
    } else {
      Serial.println("restarting rhythm");     // long pause, not a stroke
    }
  } else {
    Serial.println("first stroke, timing starts now");
  }
