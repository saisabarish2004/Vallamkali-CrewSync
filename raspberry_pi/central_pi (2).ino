...
#define PACKET_TYPE_READY  2      // band is powered, calibrated and in range
...
// How many bands are in the boat. Set to 2 if one MPU6050 is dead.
// With 2 rowers, rower 1 is the stroke seat and sets the rhythm; rower 2 is
// judged against them, because with only two rowers the data alone cannot
// say which of the pair is at fault.
#define NUM_ROWERS 3
...
// The newest stroke of every rower must fall inside this window, otherwise
// the buffers hold strokes from different moments and the offsets measure
// buffer mismatch instead of synchronisation. Roughly 2 stroke periods.
#define SYNC_WINDOW_MS    2500
...
uint32_t lastSend = 0;
uint32_t lastSeen[3] = {0, 0, 0};      // last packet of any kind, per band
bool     bandCalibrated[3] = {false, false, false};
...
void onDataReceive(const esp_now_recv_info_t *info,
  ...
  lastSeen[p.rower - 1] = millis();

  if (p.type == PACKET_TYPE_READY) {
    bandCalibrated[p.rower - 1] = (p.duration_ms == 1);
    return;                              // not a stroke, nothing to buffer
  }
...
bool batchReady() {
  uint32_t now = millis();
  for (int r = 0; r < NUM_ROWERS; r++) {
    if (have[r] < STROKES) return false;             // not enough yet
    if (now - lastStroke[r] > STALE_MS) return false; // this rower stopped
  }

  // All rowers must have stroked recently, within a couple of stroke
  // periods of each other. Otherwise the buffers are not contemporaneous.
  uint32_t newest = lastStroke[0], oldest = lastStroke[0];
  for (int r = 1; r < NUM_ROWERS; r++) {
    if (lastStroke[r] > newest) newest = lastStroke[r];
    if (lastStroke[r] < oldest) oldest = lastStroke[r];
  }
  if (newest - oldest > SYNC_WINDOW_MS) return false;
...
void sendBatchToPi() {
  ...
  // earliest stroke start across the batch becomes time zero
...
// which bands are alive, and did they calibrate
void showBands() {
  uint32_t now = millis();
  Serial.print("bands:");
  for (int r = 0; r < NUM_ROWERS; r++) {
    bool alive = lastSeen[r] && (now - lastSeen[r] < 6000);
    Serial.printf("  R%d %s%s", r + 1,
                  alive ? "ok" : "MISSING",
                  alive && !bandCalibrated[r] ? " (not calibrated)" : "");
  }
  Serial.println();
}
...
void applyDecision(int rower, int fault) {
  ...
  if (fault == 0 || rower == 0) {
    for (uint8_t r = 1; r <= NUM_ROWERS; r++)
...
void setup() {
  ...
  if (NUM_ROWERS >= 2) addRover(2, ROVER2_MAC);
  if (NUM_ROWERS >= 3) addRover(3, ROVER3_MAC);
...
void loop() {
  ...
  if (!warnedIncomplete && lastSend && now - lastSend > STALE_MS) {
    ...
    for (uint8_t r = 1; r <= NUM_ROWERS; r++)  // do not leave a band on RED
      sendDecision(r, STATUS_CORRECT, ERROR_NONE, LED_COMMAND_GREEN);
  }
}
